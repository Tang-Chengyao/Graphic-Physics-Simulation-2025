...
namespace VCX::Labs::RigidBody {
    ...
    void CaseMultipleBox::OnSetupPropsUI() {
        if (ImGui::CollapsingHeader("System", ImGuiTreeNodeFlags_DefaultOpen)) {
            ImGui::Text("Total Energy: %.4f", _energy);
    ...
    void CaseMultipleBox::HandleCollisionWallBox(const Wall & wall, Box & box) {
        ...
        for (const auto & vertexPosition : box.VerticesPosition) {
            ...
            float     dotProductVelocity = glm::dot(n, VertVelocity);
            if (dotProductPosition < 0 && dotProductVelocity < 0) {
                counter += 1;
        ...
        glm::mat3 I_world           = R * box.I * glm::transpose(R);
    ...
    void CaseMultipleBox::UpdatePhysics(float dt) {
        for (int id { 0 }; id < _numBoxes; id++) {
            ...
            glm::mat3 I_world     = R * _boxes[id].I * glm::transpose(R);
            glm::mat3 I_world_inv = glm::inverse(I_world);
            _boxes[id].w += dt * I_world_inv * (-glm::cross(_boxes[id].w, I_world * _boxes[id].w));
